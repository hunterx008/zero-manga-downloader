...
func (zd *ZeroDownload) GetComicPageInfoNew(pageURL string) *Comic {
	...
	baseUrl := pageURL[:strings.LastIndex(pageURL, "/")+1]
...
func (zd *ZeroDownload) GetComicPageInfoDetails(pageURL string) *Comic {
	...
	log.Printf("Preparing to obtain chapter information/准备获取章节信息(PC详情页): %s", comic.Title)

	chaptersRe := regexp.MustCompile(`const\s+mangaDownloadChapters\s*=\s*(\[.*?\]);`)
	matches := chaptersRe.FindStringSubmatch(bodyStr)
	if len(matches) < 2 {
		log.Fatal("Failed to extract mangaDownloadChapters from details page/无法从详情页提取章节数据")
	}

	var chapters []chapterInfo
	if err := json.Unmarshal([]byte(matches[1]), &chapters); err != nil {
		log.Fatalf("Failed to parse mangaDownloadChapters JSON/解析章节JSON失败: %s", err.Error())
	}
